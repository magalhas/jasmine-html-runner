      ...
      .finish();
	    ...
	    this.interval = setInterval(function () {
			if (self.options.timeout && self.try * 1000 >= self.options.timeout) {
				...
				page.evaluate(
					function () {
						var element = document && document.querySelector && document.querySelector(".duration");
						var done = element && element.innerText.indexOf("finished") != -1;
						return done?element.innerText:"";
					},
					function (result) {
...
JasmineHtmlRunner.prototype.saveJUnitOutput = function () {
	...
	if (this.options.outputFile) {
		this.jUnitOutput = "<?xml version=\"1.0\" encoding=\"UTF-8\" ?>\n<testsuites>"
			+ this.jUnitOutput + "\n</testsuites>";
    // Create jUnit output directory if it doesn't exist
    if (this.options.outputFile.indexOf("/") !== -1) {
      dir = this.options.outputFile.split("/");
      dir.pop();
      dir = dir.join("/");
      fs.mkdirsSync(dir);
    }
